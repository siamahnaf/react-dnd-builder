...
const Image = ({ item }: Props) => {
    ...
    //Generate Image Size
    const getImageSize = () => {
        if (item.content.size === "custom") {
            return {
                width: Number(item.content.custom_size.width),
                height: Number(item.content.custom_size.height)
            }
        } else if (item.content.size) {
            const size = item.content.size?.split("×");
            return {
        ...
        <WidgetDropper id={item.id}>
            ...
            >
                <style jsx>{`
                    ...
                    .image-box {
                        width: ${item.style.width[device.value].value}${item.style.width[device.value].unit};
                        height: ${item.style.height[device.value].value}${item.style.height[device.value].unit};
                ...
                <div
                    className={`relative image-container ${item.advance.layout.css_class}`}
