...
const ChoiceBox = ({ label, value = [], onChange, className }: Props) => {
    //Handler
    const onAddItem = () => {
    ...
    const updateItem = (field: Partial<keyof ValueProps>, index: number, newValue: string) => {
        ...
        <div className={`${className}`}>
            ...
            <div className="border border-solid border-gray-200 rounded-md mt-2.5">
                <div className="flex">
                    ...
                    <div className="text-base bg-gray-100 flex-1 px-3 py-1.5 font-medium text-gray-600">Value/ID</div>
                    ...
                    {value?.map((item, i) => (
                        ...
                        >
                            <div className="flex-1 relative">
                                <input
                                    value={item.label}
                                    onChange={(e) => updateItem("label", i, e.target.value)}
                            ...
                            <div className="flex-1 relative">
                                <input
                                    value={item.value}
                                    onChange={(e) => updateItem("value", i, e.target.value)}
                                    placeholder={`Item${i + 1}`}
                                    className="w-full px-3 py-1.5 text-gray-600 outline-none"
                                />
                                {!item.value &&
