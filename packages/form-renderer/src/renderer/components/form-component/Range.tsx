...
import { Collapse, ErrorMessage, RangeRoot } from "../common";
    ...
    return (
        ...
        >
            ...
            {general?.tooltip && general.showDescription && (!layout?.descriptionAlignment || layout?.descriptionAlignment === "u_t") &&
                <p className="text-[15px] font-light -mt-4 mb-4">{general.tooltip}</p>
            }
            <Collapse open={open}>
            ...
            {general?.tooltip && general.showDescription && layout?.descriptionAlignment === "u_i" &&
                <p className="text-[15px] font-light -mb-5 mt-5">
                    {general.tooltip}
                </p>
            }
            {general?.label && general.showTitle && layout?.titleAlignment === "bottom" &&
                <label htmlFor={id} className="block text-[17px] font-medium mt-5">{general.label}</label>
            }
        </div>
    );
};
